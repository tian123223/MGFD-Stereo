...
from core.Stereo import Stereo, autocast
...
def validate_sceneflow(model, iters=32, mixed_prec=False):
    ...
    val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=1, shuffle=False, num_workers=0, drop_last=False)
    ...
    for batch in tqdm(val_dataloader, total=len(val_dataloader)):
        _, image1, image2, flow_gt, valid_gt = [x[0] for x in batch]
        image1 = image1[None].cuda()
        image2 = image2[None].cuda()

        if torch.isnan(image1).any() or torch.isnan(image2).any():
            print(f"NaN in input images at index {time_count}")
            continue
        ...
        with autocast(enabled=False):
            start = time.time()
            flow_pr = model(image1, image2, iters=16, test_mode=True)
            end = time.time()

        if torch.isnan(flow_pr).any():
            print(f"NaN in flow_pr at index {time_count}")
            continue
        ...
        if val.sum() == 0:
            print(f"Empty val mask at index {time_count}")
            continue

        epe_val = epe[val]
        if torch.isnan(epe_val).any():
            print(f"NaN in epe_val at index {time_count}")
            continue

        epe_mean = epe_val.mean().item()
        if np.isnan(epe_mean):
            print(f"NaN EPE at index {time_count}")
            continue

        out = (epe > 3.0)
        epe_list.append(epe_mean)
        out_list.append(out[val].cpu().numpy())
        onetime = end - start
        time_total += onetime
        time_count += 1
    ...
    if time_count == 0:
        raise ValueError("No valid samples processed; time_count remains 0.")
...
if __name__ == '__main__':
    ...
    model = torch.nn.DataParallel(Stereo(args), device_ids=[0])
    ...
    if args.restore_ckpt is not None:
        ...
        for name, param in model.named_parameters():
            if torch.isnan(param).any():
                print(f"NaN found in model parameter: {name}")

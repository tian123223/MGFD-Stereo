...
class SceneFlowDatasets(StereoDataset):
    ...
    def _add_things(self, split='TRAIN'):
        ...
        root = osp.join(self.root, '/hy-tmp/sceneflow/FlyingThings3D')
    ...
    def _add_monkaa(self):
        ...
        original_length = len(self.disparity_list)
        root = osp.join(self.root, '/hy-tmp/sceneflow/Monkaa')
    ...
    def _add_driving(self):
        ...
        root = osp.join(self.root, '/hy-tmp/sceneflow/Driving')
        left_images = sorted(glob(osp.join(root, self.dstype, '*/*/*/left/*.png')))
        # print('left_images',left_images[-1])
        right_images = sorted(glob(osp.join(root, self.dstype, '*/*/*/right/*.png')))
        # right_images = [ image_file.replace('left', 'right') for image_file in left_images ]
...
class ETH3D(StereoDataset):
    def __init__(self, aug_params=None, root='/hy-tmp/ETH3D', split='training'):
...
class KITTI(StereoDataset):
    def __init__(self, aug_params=None, root='/hy-tmp/KITTI/kitti_2015', image_set='training'):
        super(KITTI, self).__init__(aug_params, sparse=True, reader=frame_utils.readDispKITTI)
        print(f"Dataset root path: '{root}' (Exists: {os.path.exists(root)})")
        assert os.path.exists(root)

        root_12 = '/hy-tmp/KITTI/kitti_2012'
        ...
        root_15 = '/hy-tmp/KITTI/kitti_2015'
...
class Middlebury(StereoDataset):
    def __init__(self, aug_params=None, root='/hy-tmp/Middlebury_HH', split='H'):

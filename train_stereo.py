...
#from core.stereo import Stereo
...
def train(args):
    model = nn.DataParallel(Stereo(args))
    #if args.gpus > 1:
        #model = torch.nn.DataParallel(Stereo(args))
            ...
            if total_steps % validation_frequency == validation_frequency - 1:
                ...
                if not args.disable_validation:
                    if 'sceneflow' in args.train_datasets:
                        ...
                        logging.warning("No validation dataset available for {}, skipping validation".format(args.train_datasets))
                        results = {}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='stereo', help="name your experiment")
    ...
    # 在其他参数后面添加
    parser.add_argument('--disable_validation', action='store_true', help='禁用训练期间的验证')
